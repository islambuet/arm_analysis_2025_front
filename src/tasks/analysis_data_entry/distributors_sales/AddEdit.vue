...
const setInputFields=async ()=>{
  ...
  inputFields[key] = {
    ...
    label: labels.get('label_'+key),
    type:'dropdown',
    options:item.data['variety_id']>0?taskData.pack_sizes.filter((temp)=>{ if(temp.variety_id==item.data['variety_id']){temp.value=temp.id.toString();temp.label=temp.name;return true}}):[],
    default:item.data[key],
    mandatory:true
  };
  key='quantity';
...
$(document).ready(function()
{
  ...
  $(document).on("change",'#variety_id',async function()
  {
    ...
    item.inputFields[key].options=taskData.pack_sizes.filter((temp)=>{ if(temp.variety_id==variety_id){temp.value=temp.id.toString();temp.label=temp.name;return true}})
    await systemFunctions.delay(1);
    $('#'+key).val('');
  })

    ...
    <div class="card-body">
      ...
      <form id="formSaveItems" v-if="item.show_report">
        ...
        <div class="row mb-2">
          ...
          <div class="col-lg-4 col-8">
            <label class="font-weight-bold">{{item.data.length}}</label>
          </div>
        </div>
...
let item=reactive({
  ...
  show_report:false,
  file_name:'',
  columns:{all:{},hidden:['sl_num','status','territory_name','type_name']},
  data:[],
  inputItems:{},
  inputFields:{},
})
...
for(let i in taskData.crop_types){
  types_object[taskData.crop_types[i]['id']]=taskData.crop_types[i];
}
        ...
        reader.onload = function(e) {
          ...
          for(let i=1;i<rows.length;i++){
            ...
            if(cols.length==num_cols){
              datum['status']='';
              datum['territory_id']=inputDatum['territory_id']=cols[0].trim()
              datum['territory_name']='NF';
              datum['type_id']=inputDatum['type_id']=cols[1].trim()
              datum['type_name']='NF';
              datum['market_size_total']=inputDatum['market_size_total']=cols[2].trim().replaceAll(',','')

              if(location_territories_object[datum['territory_id']] ){
                datum['territory_name']=location_territories_object[datum['territory_id']]['name'];
              }
              else {
                datum['status']=labels.get('label_territory_id');
              }
              if(types_object[datum['type_id']] ){
                datum['type_name']=types_object[datum['type_id']]['name'];
              }
              else {
                datum['status']=labels.get('label_type_id');
              }
              if(!datum['status']){
                data.push(datum);

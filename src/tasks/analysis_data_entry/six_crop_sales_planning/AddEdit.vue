    ...
    <div class="card-body">
      <form id="formSaveItem">
        ...
        <div v-if="item.exists">
          ...
          <div class="row mb-2">
            ...
            <div class="col-8">
              <table class="table table-bordered">
                ...
                <tr v-for="(compare_data,comp_variety_id) in item.data.compare_variety">
                  <td colspan="2">
                    <div class="row mb-2">
                      <div class="col-3">Why Recommended {{varieties_object[comp_variety_id]?varieties_object[comp_variety_id].name:'NF'}}</div>
                      <div class="col-9"><textarea class="form-control" :name="'item[compare_variety]['+comp_variety_id+'][reason_competitor_variety]'">{{compare_data['reason_competitor_variety']}}</textarea></div>
                    </div>
                    <template v-for="(arm_variety_data,arm_variety_id) in compare_data['arm_varieties']">
                      <div class="row mb-2">
                        ...
                        <div class="col-9">
                          <input type="text" class="form-control float_positive" :name="'item[compare_variety]['+comp_variety_id+'][arm_varieties]['+arm_variety_id+'][quantity_arm_variety]'" :value="arm_variety_data['quantity_arm_variety']">
                        </div>
                      </div>
                      <div class="row mb-2">
                        <div class="col-3">Why Suggested {{varieties_object[arm_variety_id]?varieties_object[arm_variety_id].name:'NF'}}</div>
                        <div class="col-9">
                          <textarea class="form-control" :name="'item[compare_variety]['+comp_variety_id+'][arm_varieties]['+arm_variety_id+'][reason_arm_variety]'">{{arm_variety_data['reason_arm_variety']}}</textarea>
                        </div>
                      </div>
                    </template>
                  </td>
          ...
          <div class="row mb-2">
            <div class="col-4">
              <label class="font-weight-bold float-right">{{labels.get('label_dealer_meeting')}}</label>
            </div>
            <div class="col-lg-4 col-8">
              <input type="text" name="item[dealer_meeting]" class="form-control integer_positive" :value="item.data.dealer_meeting">
            </div>
          </div>
          <div class="row mb-2">
            <div class="col-4">
              <label class="font-weight-bold float-right">{{labels.get('label_farmer_meeting')}}</label>
            </div>
            <div class="col-lg-4 col-8">
              <input type="text" name="item[farmer_meeting]" class="form-control integer_positive" :value="item.data.farmer_meeting">
            </div>
          </div>
          <div class="row mb-2">
            <div class="col-4">
              <label class="font-weight-bold float-right">{{labels.get('label_num_demo')}}</label>
            </div>
            <div class="col-lg-4 col-8">
              <input type="text" name="item[num_demo]" class="form-control integer_positive" :value="item.data.num_demo">
            </div>
          </div>
        </div>
      </form>
  ...
  $(document).on("click",'.btn_add_more_competitor_variety_major',function()
  {
    ...
    if(competitor_variety_id>0){
      ...
      if(arm_ids.length>0){
        let html='<tr>';
        html+='<td colspan="2">';
        html+='<div class="row mb-2"><div class="col-3">Why Recommended '+competitor_variety_name+'</div><div class="col-9"><textarea class="form-control" name="item[compare_variety]['+competitor_variety_id+'][reason_competitor_variety]"></textarea></div> </div>';
        for(let index in arm_ids){
          let arm_variety_id=arm_ids[index];
          let arm_variety_name=varieties_object[arm_variety_id]?varieties_object[arm_variety_id].name:'NF'
          html+='<div class="row mb-2"><div class="col-3">Approx Quantity '+arm_variety_name+'</div><div class="col-9"><input type="text" class="form-control float_positive" name="item[compare_variety]['+competitor_variety_id+'][arm_varieties]['+arm_variety_id+'][quantity_arm_variety]" value=""></div> </div>';
          html+='<div class="row mb-2"><div class="col-3">Why Suggested '+arm_variety_name+'</div><div class="col-9"><textarea class="form-control" name="item[compare_variety]['+competitor_variety_id+'][arm_varieties]['+arm_variety_id+'][reason_arm_variety]"></textarea></div> </div>';
        }
        html+='</td>'

      ...
      await axios.post(taskData.api_url+'/get-items',formData).then((res)=>{
        if (res.data.error == "") {
          ...
          if(options['distributor_id']>0){
            ...
            locations=taskData.distributors.filter((temp)=>{ if((temp.territory_id==options['territory_id'])&& (temp.status=='Active')){temp.location_id=temp.id;temp.label=temp.name;return true}})
          }
          else if(options['area_id']>0){
            locations=taskData.distributors.filter((temp)=>{ if((temp.area_id==options['area_id'])&& (temp.status=='Active')){temp.location_id=temp.id;temp.label=temp.name;return true}})
          }
          else if(options['part_id']>0){
            locations=taskData.location_areas.filter((temp)=>{ if((temp.part_id==options['part_id']) && (temp.status=='Active')){temp.location_id=temp.id;temp.label=temp.name;return true}})
          }
          else{
            locations=taskData.location_parts.filter((temp)=>{ if(temp.status=='Active'){temp.location_id=temp.id;temp.label=temp.name;return true}})
          }
          for(let i in locations){
            let location=locations[i];
            columns_all.push({'group':'stock_open_quantity','key':'stock_open_quantity_'+location['location_id'],'label':location['label']+'<br>('+labels.get('label_stock_open_quantity')+')','width':120})
          ...
          for(let variety_id in res.data.stock_open_quantity){
            if(rows[variety_id]){
              for(let location_id in res.data.stock_open_quantity[variety_id]){
                rows[variety_id]['stock_open_quantity_'+location_id]=(+res.data.stock_open_quantity[variety_id][location_id]);
              }
          ...
          for(let variety_id in res.data.stock_end_quantity){
            if(rows[variety_id]){
              for(let location_id in res.data.stock_end_quantity[variety_id]){
                rows[variety_id]['stock_end_quantity_'+location_id]=(+res.data.stock_end_quantity[variety_id][location_id]);
              }
            }
          }

          ...
          <div class="col-8">
            <table id="table_varieties" class="table table-bordered">
              ...
              <tr v-for="(quantity,pack_size_id) in item.data.pack_sizes" v-if="item.exists">
                ...
                <td class="quantity_kg">{{pack_sizes_object[pack_size_id]?((+pack_sizes_object[pack_size_id].value)*(+quantity)/1000):'0'}}</td>
...
$(document).ready(async function()
{
  ...
  $(document).on("click",'.btn_add_more_pack_size',function()
  {
    ...
    if(pack_size_id>0){
      ...
      html+=('<td class="quantity_kg">'+(+$('#quantity_pkt').val()/1000)*(+pack_size_value)+'</td>');
